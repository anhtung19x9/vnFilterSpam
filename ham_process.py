...
def get_all_file(dic_path):
    file_path = glob.glob(dic_path + "/ham*.txt")
    for x in file_path:
        read_file(x)
...
def process_db(text):
    ...
    # process total table for ham and ham numbers
    cur.execute("select ham,total from total")
    number = cur.fetchone()
    number_ham = number[0] + 1
    number_total = number[1] + 1
    cur.execute("update total set ham = %s, total = %s where id =1", (number_ham, number_total))
    ...
    for x in text:
        ...
        value = cur.fetchone()
        if value is None:
            cur.execute("insert into frequency values (%s,%s,%s,%s)", (x, 1, 0, 1))
        else:
            token_ham = value[1] + 1
            token_total = value[3] + 1
            cur.execute("update frequency set ham = %s, total = %s where word like %s", (token_ham, token_total, x))
...
if __name__ == '__main__':
    user_input = input("Enter path to ham files: ")
